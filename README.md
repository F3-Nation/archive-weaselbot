...
- [Coming Eventually] archive your pictures uploaded to slack onto Google Drive and / or Google Photos
...
1. If you want the achievements functionality, create a channel in your Slack workspace called `#achievements-unlocked`. Copy that new channel's `Channel ID`. You can find this on Desktop by clicking on the dropdown for the channel, the channel's ID will be at the bottom.
2. Please send `@Sumo (Chicago)` on the [F3 Nation Slack](https://f3nation.slack.com/) the following info:
...
3. I will create some additional tables in your PAXMiner schema as well as add another column to your AO table.
...
7. In order to correctly identify a man's home region, Weaselbot looks at workout activity nationwide for each email address and applies some algorithms. This is not perfect but works for almost all PAX. There are exceptions. Please notify `@Sumo (Chicago)` on the Nation Slack if you're seeing undesired behavior.
...
4. If using the paxminer database (most common use), copy `.env.example`, rename to `.env` and fill credentials (ask Moneyball for these). Otherwise, create your own local db and use your own credentials.
